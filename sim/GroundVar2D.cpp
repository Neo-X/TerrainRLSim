...
void cGroundVar2D::InitSegments(const tVector& bound_min, const tVector& bound_max)
{
	...
	for (int i = 0; i < GetNumSegments(); ++i)
	{
		...
		double bound_min = (align_mode == eAlignMax) ? -w : mBlendParams[cTerrainGen2D::eParamsInitialPaddingBoundWidth];
		double bound_max = (align_mode == eAlignMax) ? mBlendParams[cTerrainGen2D::eParamsInitialPaddingBoundWidth] : w;
								...
								eAlignMode align_mode, const tVector& fix_point)
{
	...
	if (contains_origin)
	{
		// std::cout << " Adding padding: " << seg_id << std::endl;
...
{
	std::unique_ptr<tSegment>& seg = mSegments[seg_id];
	double padding_width = mBlendParams[cTerrainGen2D::eParamsInitialPaddingWidth];
