...
class cScenarioExpImitateViz : virtual public cScenarioExpCacla
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	cScenarioExpImitateViz();
	virtual ~cScenarioExpImitateViz();
...
protected:
	...
	virtual double CalcRandTimeWarping();

	double _pose_w; // pose error weight
	double _vel_w; // pose velocity error
	double _end_eff_w; // weight for end effector error
	double _root_w; /// weight for the pose and velocity error of the root link
	double _com_w; /// com velocity error weight

	double _time_warp_min;
	double _time_warp_max;
};
