...
void cDrawCharacter::DrawShapes(const cKinSimCharacter& character, const tVector& fill_tint, const tVector& line_col)
{
...
void cDrawCharacter::DrawShape(const cCharacter& character, const cKinTree::tDrawShapeDef& def, const tVector& fill_tint, const tVector& line_col)
{
	...
	case cKinTree::eBodyShapeSphere:
		DrawShapeSphere(character, def, fill_tint, line_col);
		break;
	case cKinTree::eShapeCylinder:
		DrawShapeCylinder(character, def, fill_tint, line_col);
	break;
...
void cDrawCharacter::DrawShapeCapsule(const cCharacter& character, const cKinTree::tDrawShapeDef& def, const tVector& fill_tint, const tVector& line_col)
{
	...
	cDrawUtil::SetColor(col);
	cDrawUtil::DrawCapsule(size[1], 0.5 * size[0], cDrawUtil::eDrawSolid);

	if (line_col[3] > 0)
	{
		cDrawUtil::SetColor(line_col);
		// cDrawUtil::DrawCapsule(size[1], 0.5 * size[0], cDrawUtil::eDrawWire);
	}

	cDrawUtil::PopMatrix();
}

void cDrawCharacter::DrawShapeSphere(const cCharacter& character, const cKinTree::tDrawShapeDef& def, const tVector& fill_tint, const tVector& line_col)
{
	double theta = 0;
	tVector axis = tVector(0, 0, 1, 0);
	cKinTree::GetDrawShapeRotation(def, axis, theta);
...
void cDrawCharacter::DrawShapeCylinder(const cCharacter& character, const cKinTree::tDrawShapeDef& def, const tVector& fill_tint, const tVector& line_col)
{
	double theta = 0;
	tVector axis = tVector(0, 0, 1, 0);
	cKinTree::GetDrawShapeRotation(def, axis, theta);
