...
public:
	...
	static void LoadMatrix(const tMatrix& mat);
	static void SetProjectionMatrix(const tMatrix& mat);
	static void MatrixMode(const Util::MAT_STACK_MODE mMode);
	static Util::MAT_STACK_MODE getMatrixMode();

	static void AddShader(std::shared_ptr<cShader> shader);
	...
	static std::vector<std::shared_ptr<cShader>> gShaders;
...
protected:
	...
	static std::unique_ptr<cDrawMesh> gSphereMeshWire;
