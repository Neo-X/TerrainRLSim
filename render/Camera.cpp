...
tMatrix cCamera::BuildProjMatrix() const
{
	...
	cDrawUtil::SetProjectionMatrix(proj_mat);
	return proj_mat;
}
...
{
	Util::MAT_STACK_MODE prev_mode;
// #ifndef USE_OpenGLES
	// glGetIntegerv(GL_MATRIX_MODE, &prev_mode);
	prev_mode = cDrawUtil::getMatrixMode();
	checkError();
	cDrawUtil::MatrixMode(Util::MODELVIEW_MAT);

	tMatrix world_view = BuildWorldViewMatrix();
	cDrawUtil::LoadMatrix(world_view);

	cDrawUtil::MatrixMode(prev_mode);
// #endif
}
...
void cCamera::MouseMove(double x, double y)
{
	...
	{
#ifdef USE_OpenGLES
		std::cout << "openGL ES does not support some parts of GLUT" << std::endl;
		int mouse_mod = 0;
