	...
	cDrawUtil::SetLineWidth(1);
		...
		for (double x = min_x - std::fmod(min_x, gLineSpacing) - gLineSpacing; x < max_x + gLineSpacing; x += gLineSpacing)
		{
			...
			if (valid_sample)
			{
				...
				tVector p_ = tVector(x, ground_h, aabb_max[2], 0);
				verts_tmp.push_back(p_);
				// verts_tmp.push_back(x)
				// verts_tmp.push_back(ground_h)
				// verts_tmp.push_back(aabb_max[2])
			}
		}
		cDrawUtil::DrawLineStrip(verts_tmp);
		{
			/*
...
void cDrawGround::BuildMeshPlane(const cGround* ground, cDrawMesh* out_mesh)
{
	assert(ground->GetGroundClass() == cGround::eClassPlane
		...
		|| ground->GetGroundClass() == cGround::eClassDynamicCharacters3D
